...
/*   Updated: 2024/07/26 02:36:17 by alamaoui         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*expander(t_lexer *lexer, t_env *env, char *value)
{
	char	*string;
	...
	if (lexer->c == '?')
	{
		...
		if ((lexer->c == '$' && lexer->content[lexer->i + 1])
			|| (lexer->c == '$' && lexer->content[lexer->i + 1] != '"'))
		{
			string = if_dollar(lexer, env, NULL);
			if (string)
				value = ft_strjoin(value, string);
		}
		if (ft_printable(lexer->c))
		{
			string = norm(lexer, string);
			value = ft_strjoin(value, string);
		}
	}
	return (value);
...
char	*if_dollar(t_lexer *lexer, t_env *env, char *value)
{
	...
	lexer->str = NULL;
	lexer->string = NULL;
	value = expander(lexer, env, value);
